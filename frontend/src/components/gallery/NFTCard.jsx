    ...
    >
      <div className="nft-card-media">
        ...
        <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/75 to-transparent">
          <p className="text-[11px] text-slate-300/80">Collection: MAGA Orbit</p>
        </div>
      ...
      <div className="p-3.5 text-white space-y-2">
        <h3 className="text-base sm:text-lg font-semibold truncate text-slate-100">{nft.name}</h3>
        <p className="text-xs text-slate-400 h-8 sm:h-10 overflow-hidden line-clamp-2">{nft.description}</p>

        <div className="grid grid-cols-1 gap-2 text-[11px]">
          <div className="rounded-lg border border-[color:var(--line)] bg-black/20 px-2 py-1">
            <p className="text-slate-400">Owner</p>
            <p className="truncate font-data text-slate-200">{isOwner ? 'You' : `${nft.owner.slice(0, 6)}...${nft.owner.slice(-4)}`}</p>
          </div>
          <div className="rounded-lg border border-[color:var(--line)] bg-black/20 px-2 py-1">
            <p className="text-slate-400">Top Offer</p>
            <p className="truncate font-data text-[color:var(--brand)]">
        ...
        <div className="pt-1 flex items-center justify-between">
          <p className="font-data text-sm sm:text-base font-semibold text-[color:var(--accent-gold)]">
        ...
        <div className="pt-2">
          {isOwner ? (
            <div className="space-y-2">
              {isListed ? (
                <button type="button" onClick={(e) => { e.stopPropagation(); onOpenAction('cancel', nft); }} className="card-action-btn btn-action-danger py-2 text-xs sm:text-sm">
                  Cancel Listing
                </button>
              ) : (
                <button type="button" onClick={(e) => { e.stopPropagation(); onOpenAction('list', nft); }} className="card-action-btn btn-action-list py-2 text-xs sm:text-sm">
                  List for Sale
                </button>
              )}
              {nft.topOfferAmount > 0 && (
                <button type="button" onClick={(e) => { e.stopPropagation(); onOpenAction('accept-offer', nft); }} className="card-action-btn btn-action-accept py-2 text-xs sm:text-sm">
                  Accept Top Offer
                </button>
              )}
            </div>
            ...
            <div className="space-y-2">
              {isListed && (
                <button type="button" onClick={(e) => { e.stopPropagation(); onOpenAction('buy', nft); }} className="card-action-btn btn-action-primary py-2 text-xs sm:text-sm">
                  Buy Now
                </button>
              )}
              <button type="button" onClick={(e) => { e.stopPropagation(); onOpenAction('offer', nft); }} className="card-action-btn btn-action-bid py-2 text-xs sm:text-sm">
                Place Bid
              </button>
              {isTopBidder && (
                <button type="button" onClick={(e) => { e.stopPropagation(); onOpenAction('cancel-offer', nft); }} className="card-action-btn btn-action-danger py-2 text-xs sm:text-sm">
                  Cancel My Bid
                </button>
              )}
            </div>
